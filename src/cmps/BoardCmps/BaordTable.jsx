                    ...
                    {board.groups.map(group =>
                        group.tasks.map(task =>
                            <div className="table-row cell">
                                ...
                                <div className="group-title cell">{group.title}<span className="td-fade"></span></div>
                                {!!task.labelIds.length && <div className="board-labels cell">{getLabels(task).map(label =><div key={label.id} className={`label ${label.color || 'orange'}`}><span>{label.title}</span></div>)}<span className="td-fade"></span></div>}
                                {!!task.memberIds?.length && <div className="table-preview-avatars cell"><AvatarList users={getMembers(task)} /><span className="td-fade"></span></div>}
                                {!!task.date?.dueDate && <div className="table-date cell" id={getDateStatus(task)}><input type="checkbox"/><span>{time_icon}{getDateFormat(task)}</span><span className="td-fade"></span></div>}
                            </div>
                        )
                    )}
                </div>
