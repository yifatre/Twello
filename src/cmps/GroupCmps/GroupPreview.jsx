...
export function GroupPreview({ group, saveGroup, board, isLabelsExtended, setIsLabelExtended, saveTask, removeTask, idx }) {
    ...
    useEffect(() => {
        const el = refDrag.current;
        // invariant(el);
        return draggable({
            element: el,
            onDragStart: () => setDragging(true),
            onDrop: () => setDragging(false),
            getInitialData: () => ({idx, type:'group'})
        })
    ...
    return (
        <li key={group.id}
            ...
            ref={refDrop}
            style={{ backgroundColor: isDraggedOver ? 'red' : '' }}
        >
            ...
            {isExtended && <div ref={refDrag} key={group.id} className={`group-preview ${group.style.themeColor || 'neutral'} ${scrollRef.current?.scrollHeight > scrollRef.current?.clientHeight ? 'scrolling' : ''}`}>
